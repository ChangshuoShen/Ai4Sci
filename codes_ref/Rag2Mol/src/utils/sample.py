...
def get_init(data, model, transform, threshold):
    ...
    predictions = model.sample_init(
        ...
        ret_compose_feature=batch.retrieval_data[0].compose_feature.float(),
        ret_compose_pos=batch.retrieval_data[0].compose_pos,
        ret_idx_protein=batch.retrieval_data[0].idx_protein_in_compose,
        ret_idx_ligand=batch.retrieval_data[0].idx_ligand_ctx_in_compose,
        ret_compose_knn_edge_index=batch.retrieval_data[0].compose_knn_edge_index,
        ret_compose_knn_edge_feature=batch.retrieval_data[0].compose_knn_edge_feature,
    )
    data = data.to('cpu')
    # no frontier
    ...
    element_pred, element_prob, has_atom_prob) = [p.cpu() for p in predictions]
...
def get_next(data, model, transform, threshold):
    ...
    bond_index, bond_type, bond_prob) = [p.cpu() for p in predictions]
