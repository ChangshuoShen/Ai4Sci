...
def get_clean_res_list(res_list, use_chain, verbose=False, ensure_ca_exist=False, bfactor_cutoff=None):
    clean_res_list = []
    for res in res_list:
...
def extract_protein_structure(path, use_chain):
    parser = PDBParser(QUIET=True)
    s = parser.get_structure("x", path)
    res_list = get_clean_res_list(s.get_residues(), use_chain, verbose=False, ensure_ca_exist=True)
