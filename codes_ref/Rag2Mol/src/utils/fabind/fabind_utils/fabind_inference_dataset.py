...
class InferenceDataset(Dataset):
    def __init__(self, supp_smiles, supp_names, protein_features, preprocess_dir):
        super().__init__(None, None, None, None)
        
        # read preprocessed data
        protein_esm_feature, protein_structure = protein_features
        
        self.data = []
        for smiles, name in tqdm(zip(supp_smiles, supp_names)):
            input_dict = {}

            mol, molecule_info = torch.load(os.path.join(preprocess_dir, 'mol', name+'.pt'))
    ...
    def get(self, idx):
        ...
        data.idx = idx
        data.uid = input_dict['complex_name']
