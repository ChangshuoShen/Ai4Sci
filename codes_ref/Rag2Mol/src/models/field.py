...
class SpatialClassifierVN(Module):
    ...
    def __init__(self, num_classes, num_bond_types, in_sca, in_vec, num_filters, edge_channels, num_heads, cutoff=10.0):
        super().__init__()
    ...
    def forward(self, pos_query, edge_index_query, pos_compose, h_compose, edge_index_q_cps_knn, 
        ...
        h = self.message_module(h_compose, edge_ij, edge_index_q_cps_knn[1], dist_ij, annealing=True)
        ...
        if (len(edge_index_query) != 0) and (edge_index_query.size(1) > 0):
            ...
            node_mol_j = [
                h_compose[0][idx_node_j],
                h_compose[1][idx_node_j]
            ]
