...
from .embedding import GVP, AtomEmbedding
from .interaction import CFTransformerEncoderVN
from .field import SpatialClassifierVN
...
from .support import Support_model_CA
...
def embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
    ...
    h_protein = protein_res_emb(compose_feature[idx_protein], compose_pos[idx_protein])
...
class Rag2Mol(Module):
    ...
    def __init__(self, config, num_classes, num_bond_types, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        self.protein_res_emb = AtomEmbedding(protein_atom_feature_dim, 1, *self.emb_dim)
        self.ligand_atom_emb = AtomEmbedding(ligand_atom_feature_dim, 1, *self.emb_dim)
        self.protein_res_emb_ret = AtomEmbedding(protein_atom_feature_dim, 1, *self.emb_dim)
        self.ligand_atom_emb_ret = AtomEmbedding(ligand_atom_feature_dim, 1, *self.emb_dim)
        

        self.encoder = CFTransformerEncoderVN(hidden_channels = [config.encoder.hidden_channels, config.encoder.hidden_channels_vec],
                    edge_channels = config.encoder.edge_channels, num_interactions = config.encoder.num_interactions, cutoff = config.encoder.cutoff)
        self.encoder_ret = CFTransformerEncoderVN(hidden_channels = [config.encoder.hidden_channels, config.encoder.hidden_channels_vec],
                    edge_channels = config.encoder.edge_channels, num_interactions = config.encoder.num_interactions, cutoff = config.encoder.cutoff)

        
        in_sca, in_vec = self.encoder.out_sca, self.encoder.out_vec # 256, 64

        self.support_model = Support_model_CA(in_sca, in_vec, 48)

        self.field = SpatialClassifierVN(num_classes = num_classes, num_bond_types = num_bond_types, in_vec = in_vec, in_sca = in_sca,
                    num_filters = [config.field.num_filters, config.field.num_filters_vec], edge_channels = config.field.edge_channels,
                    num_heads = config.field.num_heads, cutoff = config.field.cutoff)
        ...
        self.pos_predictor = PositionPredictor(in_sca=in_sca, in_vec=in_vec,
                                            num_filters=[config.position.num_filters]*2, n_component=config.position.n_component) #(256, 64, [128, 128], 3)
    ...
    def forward(self, pos_real, y_real, pos_fake, # query real positions,
                       ...
                       compose_batch_idx, ret_batch_idx, ret_compose_feature, ret_compose_pos, ret_idx_ligand, ret_idx_protein, ret_compose_knn_edge_index, ret_compose_knn_edge_feature,
                       ):
        ...
        ret_compose = embed_compose(ret_compose_feature, ret_compose_pos, ret_idx_ligand, ret_idx_protein,
                                self.ligand_atom_emb_ret, self.protein_res_emb_ret, self.emb_dim)
        ret_compose = self.encoder_ret(
                    node_attr = ret_compose,
                    pos = ret_compose_pos,
                    edge_index = ret_compose_knn_edge_index,
                    edge_feature = ret_compose_knn_edge_feature,
                ) 

        torch.set_printoptions(profile="full")
        h_compose = self.support_model(h_compose, compose_pos, compose_batch_idx, ret_compose, ret_compose_pos, ret_batch_idx)
    ...
    def sample_init(self,
        ...
        ret_compose_feature=None,
        ret_compose_pos=None,
        ret_idx_protein=None,
        ret_idx_ligand=None,
        ret_compose_knn_edge_index=None,
        ret_compose_knn_edge_feature=None
        ):
        ...
        focal_resutls = self.sample_focal(compose_feature, compose_pos, idx_ligand, idx_protein, compose_knn_edge_index, compose_knn_edge_feature,
                                    ret_compose_feature, ret_compose_pos, ret_idx_ligand, ret_idx_protein, ret_compose_knn_edge_index, ret_compose_knn_edge_feature, 
                                    )
    ...
    def sample_focal(self,
            ...
            ret_idx_ligand,
            ret_idx_protein,
            ret_compose_knn_edge_index,
            ret_compose_knn_edge_feature,
        ...
        h_compose = self.support_model(h_compose, compose_pos, torch.zeros(len(compose_pos)).long(), ret_compose, ret_compose_pos, torch.zeros(len(ret_compose_pos)).long())
    ...
    def sample_init_element(self, 
        ...
        #NOTE: Only one parent batch (one compose graph) at a time (i.e. batch size = 1)
        n_query = len(pos_generated)
        query_compose_knn_edge_index = knn(x=compose_pos, y=pos_generated, k=32, num_workers=16)
    ...
    def query_position(self, pos_query, h_compose, compose_pos,
        ...
        query_compose_knn_edge_index = knn(x=compose_pos, y=pos_query, k=32, num_workers=16)
